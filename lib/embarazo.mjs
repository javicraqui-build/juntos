...
export function avisos(doc, role, t){
  ...
  for (const a of doc.appointments || []) {
    ...
    if (d === manana) out.push({ key:`cita-${a.id}-manana`, title:`Mañana${hora ? ' a las ' + hora : ''}: ${a.title}`, body:`${sub ? sub + '. ' : ''}${nq ? `Llevan ${nq} ${nq === 1 ? 'pregunta anotada' : 'preguntas anotadas'}.` : 'Todavía no hay preguntas anotadas: es buen momento para pensarlas juntos.'}`, url:'/?tab=salud' });
    else if (d === t) out.push({ key:`cita-${a.id}-hoy`, title:`Hoy${hora ? ' a las ' + hora : ''}: ${a.title}`, body:`${sub ? sub + '. ' : ''}${nq ? `Repasen las ${nq} preguntas antes de entrar.` : 'Vayan con las preguntas pensadas.'}`, url:'/?tab=salud' });
  }
