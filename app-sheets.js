...
function openPerfil(){
  ...
  openSheet(`<h2>${esc(quien('mother'))} y ${esc(quien('partner'))}</h2><p class="sub">Estás usando la app como <b>${esc(quien(yo()))}</b>. <button class="link" onclick="L.role=null; saveLocal(); closeSheet(); render()">Cambiar</button></p>
    <div class="card" style="margin-bottom:14px"><span class="eyebrow">Compartir con tu pareja</span><p class="sub" style="margin-top:4px">Los dos usan el mismo espacio. Comparte esta página con el código:</p><div class="code">${esc(p.inviteCode)}</div><div class="sync ${dbOn?'on':''}" id="sync"><i></i><span>${dbOn ? 'Sincronizado con tu pareja' : 'Guardado en este dispositivo'}</span></div></div>
...
function renderOnboarding(){
  ...
  if (OB.step === 0) return `<div class="onb"><div class="grow"></div><div class="logo">juntos</div><p class="tag">El embarazo, juntos.</p>
    ...
    <div style="display:flex;justify-content:space-between;margin-top:14px"><button class="link" onclick="OB.join=true; ob(6)">Tengo un código de mi pareja</button><button class="link" style="color:var(--ink3)" onclick="S=demoWorkspace(); L.role=null; saveLocal(); commit()">Ver con datos de ejemplo</button></div>
