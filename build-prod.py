...
sheets = rep(sheets, """Comparte esta página y el código. En el móvil de tu pareja, al abrir, elige "Tengo un código de mi pareja".""",
                     """Envía la invitación a tu pareja: entra con su correo y usa el código. Los dos veréis y editaréis lo mismo.""")
...
sheets = rep(sheets, """<div class="sync ${dbOn?'on':''}" id="sync"><i></i><span>${dbOn ? 'Sincronizado con tu pareja' : 'Guardado en este dispositivo'}</span></div></div>""",
                     """<div class="sync on" id="sync"><i></i><span>Sincronizado en la nube</span></div><div style="margin-top:10px"><button class="btn ghost sm" onclick="copiarInvitacion()">Copiar invitación</button></div></div>""")
